...
def seed_reservations(data: SeedReservationsRequest, db: Session = Depends(get_db)):
    ids = ReservationService.seed(db, data.quantity)
    return SeedReservationsResponse(ids=ids, count=len(ids))
